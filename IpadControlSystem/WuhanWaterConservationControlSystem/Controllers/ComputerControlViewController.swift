//
//  ComputerControlViewController.swift
...
class ComputerControlViewController: UIViewController {
    ...
    private var computerDevices: [Device] = []
    ...
    private let customDevicesKey = "computer_custom_devices"
    private let collapseStatesKey = "computer_collapsed_states"
    private let tileStatesKey = "computer_tile_states"
    private let deletedPresetTitlesKey = "computer_deleted_preset_titles"
    private let deletedPresetGroupsKey = "computer_deleted_preset_groups"
    ...
    private func setupTableView() {
        ...
        tableView.register(ComputerDeviceCell.self, forCellReuseIdentifier: "ComputerDeviceCell")
    ...
    private func setupHexPanel() {
        ...
        for name in orderedGroupNames {
            let sectionIcon: UIImage? = {
                if #available(iOS 13.0, *) { return UIImage(systemName: "desktopcomputer") }
                return nil
            }()
            ...
            if let arr = groups[name] {
                for d in arr {
                    combined.append(TileData(title: d.name, deviceId: d.id, onHex: nil, offHex: nil, iconName: "desktopcomputer"))
                }
            }
            ...
            for td in combined {
                ...
                let deviceInitial = computerDevices.first(where: { $0.name == td.title })?.isOn ?? false
    ...
    private func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            allOnButton.heightAnchor.constraint(equalToConstant: 56)
        ])
        ...
        if #available(iOS 11.0, *) {
            allOnButton.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -30).isActive = true
        } else {
            allOnButton.bottomAnchor.constraint(equalTo: bottomLayoutGuide.topAnchor, constant: -30).isActive = true
        }
    }
    
    // MARK: - Data Loading
    
    private func loadComputerDevices() {
        isLoading = true
        
        // 获取所有电脑设备
        computerDevices = DeviceManager.shared.getDevices(byType: .computer)
    ...
    private func refreshDeviceStatus() {
        DeviceManager.shared.refreshDeviceStatus { [weak self] success in
            if success {
                self?.computerDevices = DeviceManager.shared.getDevices(byType: .computer)
                self?.tableView.reloadData()
                self?.clearStackView(self!.hexPanelStackView)
                self?.setupHexPanel()
            }
    ...
    @objc private func allOnButtonTapped() {
        guard !computerDevices.isEmpty else {
            showAlert(title: "提示", message: "没有可控制的电脑设备")
            return
        }
        
        let alert = UIAlertController(title: "确认操作", message: "确定要开启所有电脑设备吗？", preferredStyle: .alert)
        
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
        alert.addAction(UIAlertAction(title: "确定", style: .default) { [weak self] _ in
            self?.turnOnAllComputerDevices()
        })
        
        present(alert, animated: true)
    }
    
    private func turnOnAllComputerDevices() {
        isLoading = true
        allOnButton.isEnabled = false
        allOnButton.setTitle("正在执行...", for: .normal)
        
        DeviceManager.shared.controlAllDevices(ofType: .computer, action: .allOn) { [weak self] success, error in
            DispatchQueue.main.async {
                self?.isLoading = false
                ...
                if success {
                    self?.showAlert(title: "成功", message: "所有电脑设备已开启")
                    self?.refreshDeviceStatus()
                } else {
    ...
    private func groupName(for name: String) -> String {
        if name.contains("主控") { return "主控电脑" }
        if name.contains("展示") { return "展示电脑" }
        if name.contains("演示") { return "演示电脑" }
        return "其他"
    }
    ...
    private func allGroupNames() -> [String] {
        var set = Set<String>()
        for d in computerDevices { set.insert(groupName(for: d.name)) }
    ...
    private func presentIconSelector() {
        ...
        let icons: [(String, String)] = [("电脑", "desktopcomputer"), ("投影", "video.fill"), ("灯光", "lightbulb.fill"), ("电源", "power")]
    ...
    private func presentDeviceForm() {
        ...
        alert.addAction(UIAlertAction(title: "创建", style: .default, handler: { _ in
            ...
            let icon = self.pendingIconSymbol ?? "desktopcomputer"
...
extension ComputerControlViewController: UITableViewDataSource {
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return computerDevices.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "ComputerDeviceCell", for: indexPath) as? ComputerDeviceCell else {
            return UITableViewCell()
        }
        
        let device = computerDevices[indexPath.row]
...
extension ComputerControlViewController: UITableViewDelegate {
...
// MARK: - ComputerDeviceCell

class ComputerDeviceCell: UITableViewCell {
    ...
    private func setupIconImageView() {
        ...
        iconImageView.tintColor = UIColor(red: 0.2, green: 0.6, blue: 1.0, alpha: 1.0)
    ...
    private func createFallbackIcon() -> UIImage? {
        ...
        context.setFillColor(UIColor(red: 0.2, green: 0.6, blue: 1.0, alpha: 1.0).cgColor)
        context.setStrokeColor(UIColor(red: 0.2, green: 0.6, blue: 1.0, alpha: 1.0).cgColor)
        
        // 绘制显示器形状
        context.fill(CGRect(x: 4, y: 6, width: 24, height: 16))
        context.fill(CGRect(x: 12, y: 22, width: 8, height: 2))
